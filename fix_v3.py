#!/usr/bin/env python3'
"""
Fix remaining presets by extracting Preset_Notes properly.
Key fix: FIRST line is ALWAYS part of value (not a continuation line).
Only check for " prefix on SUBSEQUENT lines.
"""
import json'
...
for prog in no_desc:'
    ...
    # Construct full path'
    if preset_file.startswith('downloads/wine_spooky'):'
        file_path = Path(preset_file)'
    else:'
        file_path = Path('downloads/wine_spooky') / preset_file'
    ...
    try:'
        ...
        search_start = pos + len('Preset_Notes="')'
        quote_pos = content.find('"', search_start)'
        if quote_pos == -1:'
            continue'
        ...
        # Collect lines - first line is ALWAYS part of value'
        ...
        for i, line in enumerate(lines):'
            if i == 0:'
                # First line is ALWAYS part of the value'
                notes_lines.append(line)'
            else:'
                # Subsequent lines: if starts with ", it's a continuation'
                if line.startswith('"'):'
